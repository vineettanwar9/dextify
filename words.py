...
def _text_detect(img, image, join=False):
    ...
    kernel = np.ones((5, 30), np.uint8)  ### (4, 70) for line segmention  (5,30) for word segmentation
    img_erosion = cv2.erode(small, kernel, iterations=3)
...
def textDetectWatershed(thresh):
    ...
    for mark in np.unique(markers):
        ...
        # Limits for text
        if r > 0.2 and 2000 > w > 15 and 1400 > h > 15:
